...
public class CCDMHelperComponent {
    ...
    public static void translateToPairs(ArrayList<CCDMHelperComponent> listComp, ArrayList<NodeCCDM> listPair){
        for(int i=0;i<listComp.size();i++){
            if(listComp.get(i).componentInfo!='A'){
                ...
                CachedStorageILB.listCCDMPairs.add(e);
            }
        }
    }
    public static void propagateToPairNode(NodeCCDM e, CCDMHelperComponent c1, int number){
        ...
        if(!e.params.containsKey(KeysDictionary.X)){
            if(c1.x!=0) {
                ...
                //050612.55+372122.2
                //00018+6437
                float h = Integer.parseInt(c1.coord_I2_1fake) / 10 * 100 + Integer.parseInt(c1.coord_I2_1fake) % 10 * 6;
