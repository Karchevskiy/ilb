...
public class CustomWriter extends CachedStorageILB implements SharedConstants,KeysDictionary {
    public static void writeAllCachedData(String xxx, CachedStorage cachedStorage){
        ...
        try{
            ...
            for(int k = 0; k< cachedStorage.getSysList().size(); k++){
                writeSystem(cachedStorage.getSysList().get(k),outer);
                for(int j = 0; j< cachedStorage.getSysList().get(k).pairs.size(); j++){
                    writePairLayer(cachedStorage.getSysList().get(k).pairs.get(j),outer);
                }
            }
            outer.flush();
        }catch(Exception e){
