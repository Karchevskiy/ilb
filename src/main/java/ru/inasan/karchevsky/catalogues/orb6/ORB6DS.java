...
public class ORB6DS implements Datasource {
    ...
    public void propagate(Pair e, NodeForParsedCatalogue nodeRaw) throws Exception {
        if (nodeRaw instanceof NodeORB6) {
            NodeORB6 node = (NodeORB6) nodeRaw;
            e.addMappedEntity(NodeORB6.uniqueCatalogueID, nodeRaw.source);
            e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.MODIFIERS, "   o");
            if (node.params.containsKey(KeysDictionary.HIP) && !node.params.get(KeysDictionary.HIP).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.HIP, node.params.get(KeysDictionary.HIP));
            if (node.params.containsKey(KeysDictionary.ADS) && !node.params.get(KeysDictionary.ADS).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.ADS, node.params.get(KeysDictionary.ADS));
            if (node.params.containsKey(KeysDictionary.FLAMSTEED) && !node.params.get(KeysDictionary.FLAMSTEED).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.FLAMSTEED, node.params.get(KeysDictionary.FLAMSTEED));
            if (node.params.containsKey(KeysDictionary.BAYER) && !node.params.get(KeysDictionary.BAYER).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.BAYER, node.params.get(KeysDictionary.BAYER));
            if (node.params.containsKey(KeysDictionary.DM) && !node.params.get(KeysDictionary.DM).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.DM, node.params.get(KeysDictionary.DM));
            if (node.params.containsKey(KeysDictionary.HD) && !node.params.get(KeysDictionary.HD).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.HD, node.params.get(KeysDictionary.HD));
            if (node.params.containsKey(KeysDictionary.OBSERVER) && !node.params.get(KeysDictionary.OBSERVER).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.OBSERVER, node.params.get(KeysDictionary.OBSERVER));
            if (node.params.containsKey(KeysDictionary.WDSSYSTEM) && !node.params.get(KeysDictionary.WDSSYSTEM).equals(""))
                e.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.WDSSYSTEM, node.params.get(KeysDictionary.WDSSYSTEM));
    ...
    //clone from DatasourceWDS TODO:refactor: encapsulate in special class
    public static void parseCoordinatesDirectlyFromWDS(Pair e, NodeORB6 node) {
        ...
        try {
            e.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.X, node.el1.rightCoordX);
            e.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.Y, node.el1.rightCoordY);
            e.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.RHO, 0d);
            e.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.THETA, 0d);


            e.el1.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.X, node.el1.rightCoordX);
            e.el1.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.Y, node.el1.rightCoordY);
            e.el2.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.X, node.el2.rightCoordX);
            e.el2.addCoordinates(NodeORB6.uniqueCatalogueID, KeysDictionary.Y, node.el2.rightCoordY);


            e.el1.addMappedEntity(NodeORB6.uniqueCatalogueID, node.source);
            e.el1.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.WDSSYSTEM, node.params.get(KeysDictionary.WDSSYSTEM));
            e.el2.addMappedEntity(NodeORB6.uniqueCatalogueID, node.source);
            e.el2.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.WDSSYSTEM, node.params.get(KeysDictionary.WDSSYSTEM));
    ...
    public static void extractNameForComponents(String nameOfPair, Pair pair) {
        ...
        try {
            pair.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.WDSPAIR, nameOfPair);
            pair.el1.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.WDSCOMP, name1);
            pair.el2.addParams(NodeORB6.uniqueCatalogueID, KeysDictionary.WDSCOMP, name2);
        } catch (ValueAlreadyExistsException exc) {
            //exc.printStackTrace();
        }
    }
