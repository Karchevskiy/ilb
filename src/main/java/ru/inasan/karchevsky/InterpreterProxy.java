...
public class InterpreterProxy extends CachedStorageILB {
    ...
    public void interprWDS() {
        for (NodeWDS listWD : listWDS) {
            ...
            getS().data = listWD.coordinatesFromWDSasString;
            getS().params.put(KeysDictionary.WDSSYSTEM, listWD.params.get(KeysDictionary.WDSSYSTEM));
            getS().coordinatesNotFoundInWDS = listWD.coordinatesNotFoundInWDS;
            ...
            int h = sysList.size();
            for (int j = 0; j < h; j++) {
                if (getS().params.get(KeysDictionary.WDSSYSTEM).equals(sysList.get(j).params.get(KeysDictionary.WDSSYSTEM))) {
                    uniqueSystem = false;
                    sysNumber = j;
                    break;
                }
        ...
        System.out.println(s.params.get(KeysDictionary.WDSSYSTEM));
    }
